...
<div class="max-w-2xl mx-auto py-10 sm:px-6 lg:px-8">
    <div class="bg-white p-8 rounded-2xl border-[3px] border-black shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] hover:shadow-[0_15px_50px_-15px_rgba(0,0,0,0.25)] transition-shadow duration-300">
        <h1 class="text-2xl font-bold mb-6 text-gray-800 text-center">
            <i class="fas fa-car mr-2" style="color: #05325f;"></i> Edit Service
        </h1>
        ...
        <form action="{{ route('services.update', $service->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $service->tanggal->format('Y-m-d')) }}"
            ...
            <div>
                ...
                <input type="text" name="nama_pelanggan" id="nama_pelanggan" value="{{ old('nama_pelanggan', $service->nama_pelanggan) }}"
                    class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200 p-2"
            ...
            <div>
                ...
                <input type="text" name="plat_nomor" id="plat_nomor" value="{{ old('plat_nomor', $service->plat_nomor) }}"
            ...
            <div class="border border-gray-200 rounded-lg p-4">
                ...
                <div id="jasa-container">
                    @foreach($service->detailJasa as $index => $jasa)
                    <div class="jasa-item space-y-4 mb-4">
                        <div class="grid grid-cols-3 gap-4">
                            <div class="col-span-2">
                                <select name="service_jobs[{{ $index }}][id]" class="jasa-select select2 mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200 p-2" required>
                                    ...
                                    @foreach($serviceJobs as $job)
                                        <option value="{{ $job->id }}" data-harga="{{ $job->harga_jual }}" data-tipe="{{ $job->tipe_harga }}"
                                            {{ $jasa->service_job_id == $job->id ? 'selected' : '' }}>
                            ...
                            <div>
                                <input type="number" name="service_jobs[{{ $index }}][jumlah_jam]" class="jumlah-jam mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200 p-2"
                                    placeholder="Jumlah Jam" min="0.1" step="0.1" value="{{ old('service_jobs.'.$index.'.jumlah_jam', $jasa->jumlah_jam) }}" required>
                            </div>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-medium">Subtotal: <span class="subtotal">Rp {{ number_format($jasa->subtotal, 0, ',', '.') }}</span></span>
            ...
            <div class="border border-gray-200 rounded-lg p-4">
                ...
                <div id="barang-container">
                    @foreach($service->detailBarang as $index => $barang)
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <div class="col-span-2">
                                <select name="barangs[{{ $index }}][id]" class="barang-select select2 mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200 p-2">
                                    ...
                                    @foreach($barangs as $brg)
                                        <option value="{{ $brg->id }}" data-harga="{{ $brg->harga_jual }}" data-stok="{{ $brg->stok }}"
                                            {{ $barang->barang_id == $brg->id ? 'selected' : '' }}>
                                            {{ $brg->kode_barang }} {{ $brg->nama_barang }} (Stok: {{ $brg->stok + $barang->jumlah }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <input type="number" name="barangs[{{ $index }}][jumlah]" class="jumlah-barang mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200 p-2"
                                    placeholder="Jumlah" min="1" value="{{ old('barangs.'.$index.'.jumlah', $barang->jumlah) }}">
                            </div>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-medium">Subtotal: <span class="subtotal">Rp {{ number_format($barang->subtotal, 0, ',', '.') }}</span></span>
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="3"
                    class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200 p-2"
                    placeholder="Catatan tambahan tentang service">{{ old('keterangan', $service->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                <h3 class="font-bold text-lg text-right">Total: <span id="total-service">Rp {{ number_format($service->total, 0, ',', '.') }}</span></h3>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        let jasaIndex = {{ count($service->detailJasa) }};
        let barangIndex = {{ count($service->detailBarang) }};
        ...
        $('#tambah-jasa').click(function() {
            ...
            $('.select2').select2(); // Inisialisasi Select2 untuk elemen baru
            jasaIndex++;
        });
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div class="col-span-2">
                        <select name="barangs[{{ $index }}][id]" class="barang-select select2 mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-200 p-2">
                            ...
                            @foreach($barangs as $brg)
                                <option value="{{ $brg->id }}" data-harga="{{ $brg->harga_jual }}" data-stok="{{ $brg->stok }}"
                                    {{ (isset($barang) && $barang->barang_id == $brg->id) ? 'selected' : '' }}>
                                    {{ $brg->kode_barang }} {{ $brg->nama_barang }} - Stok: {{ isset($barang) ? ($brg->stok + $barang->jumlah) : $brg->stok }}
                                </option>
                            @endforeach
                        </select>
                    </div>
